<!DOCTYPE html>
<html>
    <?php include('../constants.php'); ?>
    <head>
        <meta charset="utf-8">
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <meta http-equiv="X-UA-Compatible" content="IE=edge">

        <title>Collapsible sidebar using Bootstrap 3</title>

         <!-- Bootstrap CSS CDN -->
        <link rel="stylesheet" href="../assets/bootstrap337/css/bootstrap.min.css">
        <!-- Our Custom CSS -->
        <link rel="stylesheet" type="text/css" href="<?php echo SITE_URL;?>professional-profile/style/style.css?version=2">
        <link rel="stylesheet" type="text/css" href="<?php echo SITE_URL;?>professional-profile/style/timeline.css">
        <link rel="stylesheet" type="text/css" href="<?php echo SITE_URL;?>professional-profile/style/editprofile.css">

         <!-- jQuery CDN -->
         <script src="https://code.jquery.com/jquery-1.12.0.min.js"></script>
         <script src="https://code.jquery.com/ui/1.12.1/jquery-ui.js"></script>

         <!-- Bootstrap Js CDN -->
         <script src="../assets/bootstrap337/js/bootstrap.min.js"></script>
         <link href="//maxcdn.bootstrapcdn.com/font-awesome/4.2.0/css/font-awesome.min.css" rel="stylesheet">
         <script src="<?php echo SITE_URL;?>platform/js/core.js"></script>
         <link rel="stylesheet" href="<?php echo SITE_URL;?>professional-profile/js/timeline.js">
         <script src="<?php echo SITE_URL;?>professional-profile/js/professional-profile.js"></script>

       

    </head>
    <body>
   